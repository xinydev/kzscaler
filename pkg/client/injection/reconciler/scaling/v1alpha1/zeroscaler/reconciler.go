...
// controller reconciling v1alpha1.ZeroScaler.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.ZeroScaler. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.ZeroScaler) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ZeroScaler.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.ZeroScaler. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.ZeroScaler) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.ZeroScaler if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.ZeroScaler.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.ZeroScaler) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.ZeroScaler) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.ZeroScaler resources.
type reconcilerImpl struct {
	...
	Lister scalingv1alpha1.ZeroScalerLister
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ZeroScalers(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.ZeroScaler, desired *v1alpha1.ZeroScaler) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ScalingV1alpha1().ZeroScalers(desired.Namespace)
		...
		updater := r.Client.ScalingV1alpha1().ZeroScalers(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ZeroScaler) (*v1alpha1.ZeroScaler, error) {

	getter := r.Lister.ZeroScalers(resource.Namespace)
	...
	patcher := r.Client.ScalingV1alpha1().ZeroScalers(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ZeroScaler) (*v1alpha1.ZeroScaler, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.ZeroScaler, reconcileEvent reconciler.Event) (*v1alpha1.ZeroScaler, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
